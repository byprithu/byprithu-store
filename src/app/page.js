...
// ── short link: store poster as ?p=<posterId>&s=<size> ───────────
function buildLink(base, poster, size) {
  if (!poster || !base) return null
  const params = new URLSearchParams({ p: poster.id, s: size || poster.size || '' })
...
function SellerDashboard() {
  ...
  const [selSize,   setSelSize]   = useState('')   // manual size for selected poster
  const [orders,    setOrders]    = useState([])
  const [loading,   setLoading]   = useState(true)
  const [copied,    setCopied]    = useState(false)
  const [adding,    setAdding]    = useState(false)
  const [saving,    setSaving]    = useState(false)
  const [newP,      setNewP]      = useState({ title:'', size:'', price:'', emoji:'🎨', bg:'#fce4ec' })
    ...
    ;(async () => {
      try {
        ...
        if (!snap.empty) {
          ...
          for (const p of DEFAULT_POSTERS) await setDoc(doc(db, 'posters', p.id), p)
        }
      } catch { setDbOk(false) }
    })()
  }, [])
  ...
  async function addPoster() {
    ...
    setSaving(true)
    const p = { ...newP, created_at: Date.now() }
    try {
    ...
    setNewP({ title:'', size:'', price:'', emoji:'🎨', bg:'#fce4ec' })
  }
  ...
  const link     = buildLink(baseUrl, selected, selSize)
    ...
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", maxWidth:580, margin:'0 auto', padding:'24px 16px 80px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
        ...
        <div style={{ marginLeft:'auto', display:'flex', alignItems:'center', gap:8 }}>
          {dbOk
            ? <span style={{ fontSize:11, background:'#e8f5e9', color:'#2e7d32', border:'1px solid #c8e6c9', borderRadius:20, padding:'3px 9px', fontWeight:700 }}>🔥 Live</span>
      ...
      {tab === 'share' && <>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(128px,1fr))', gap:10, marginBottom:20 }}>
          {posters.map(p => (
            <div key={p.id} onClick={() => pickPoster(p)} style={{ ...T.card, border:selected?.id===p.id?'2px solid #dc2743':'1.5px solid #efefef', background:selected?.id===p.id?'#fff8f9':'#fff', cursor:'pointer' }}>
              ...
              <div style={{ fontSize:11, color:'#aaa' }}>{p.price}</div>
        ...
        {selected && (
          <div style={{ marginTop:4 }}>
            <span style={T.label}>2 — Set size for this order</span>
            <div style={{ display:'flex', alignItems:'center', gap:10, background:'#fff8f9', border:'1.5px solid #f5b3c0', borderRadius:10, padding:'10px 14px', marginBottom:14 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:12, color:'#aaa' }}>{selected.price}</div>
              </div>
            </div>
            <input
              value={selSize}
              onChange={e => setSelSize(e.target.value)}
              placeholder="e.g. A4, A3, 12×18 inches, Custom 30×40 cm…"
              style={{ ...T.inp(false), marginBottom:16 }}
            />

            <span style={T.label}>3 — Share this link</span>
            <div style={{ background:'#f7f7f7', border:'1px solid #ebebeb', borderRadius:8, padding:'10px 12px', fontSize:13, color:'#555', fontFamily:'monospace', wordBreak:'break-all', marginBottom:12, lineHeight:1.7 }}>
              {link || 'Select a poster above'}
            </div>
            ...
            <div style={{ fontSize:12, color:'#bbb', lineHeight:1.8 }}>Link is short and <strong style={{ color:'#888' }}>never changes</strong>. Orders sync to all devices via Firebase.</div>
          </div>
        )}
      </>}
              ...
              <div key={o._id} style={{ ...T.card, cursor:'pointer' }} onClick={()=>setExpanded(expanded===o._id?null:o._id)}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:12, color:'#888' }}>{o.poster?.title} · {o.size} · {o.poster?.price}</div>
...
// CUSTOMER ORDER FORM
// ════════════════════════════════════════════════════════════════
function CustomerForm({ poster, size }) {
  const [form, setForm] = useState({ name:'', email:'', phone:'', addr1:'', addr2:'', city:'', state:'', pin:'', country:'India', notes:'' })
    ...
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", maxWidth:460, margin:'0 auto', padding:'24px 16px 60px' }}>
      ...
      {poster && (
        <div style={{ display:'flex', alignItems:'center', gap:10, background:'#fff8f9', border:'1.5px solid #f5b3c0', borderRadius:10, padding:'10px 14px', marginBottom:22 }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:14, color:'#111' }}>Ordering: {poster.title}</div>
            <div style={{ fontSize:12, color:'#aaa' }}>{size ? `${size} · ` : ''}{poster.price}</div>
          </div>
        </div>
      )}
...
// ════════════════════════════════════════════════════════════════
// ROOT — reads ?p=posterId&s=size from URL
// ════════════════════════════════════════════════════════════════
function AppRoot() {
  const [mode,    setMode]    = useState(null)
  const [poster,  setPoster]  = useState(null)
  const [size,    setSize]    = useState('')
  const [allPosters, setAllPosters] = useState(DEFAULT_POSTERS)

  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const posterId = params.get('p')
    const sizeParam = params.get('s') || ''

    if (posterId) {
      // load posters from firebase to find the right one
      getDocs(collection(db, 'posters')).then(snap => {
        const list = snap.empty ? DEFAULT_POSTERS : snap.docs.map(d => ({ ...d.data(), id: d.id }))
        const found = list.find(p => p.id === posterId) || DEFAULT_POSTERS.find(p => p.id === posterId)
        setPoster(found || null)
        setSize(sizeParam)
        setMode('customer')
      }).catch(() => {
        setSize(sizeParam); setMode('customer')
      })
    } else {
      setMode('seller')
    }
  ...
  if (!mode) return null
  if (mode === 'customer') return <CustomerForm poster={poster} size={size} />

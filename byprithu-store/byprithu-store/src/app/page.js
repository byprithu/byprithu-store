...
const EMOJI_OPTIONS = ['🎨','🖼','🌺','🦋','⭐','🎭','🌸','🔥','🌙','🦚']
const BG_OPTIONS = ['#fce4ec','#f3e5f5','#e8eaf6','#e1f5fe','#e8f5e9','#fff8e1','#fbe9e7','#fff3e0']

const DEFAULT_POSTERS = [
  { id: 1, title: 'Vintage Sunset',  size: 'A4', price: '₹299', emoji: '🌅', bg: '#fff3e0' },
  { id: 2, title: 'Botanical Bloom', size: 'A3', price: '₹499', emoji: '🌿', bg: '#e8f5e9' },
  { id: 3, title: 'Cosmic Dreams',   size: 'A4', price: '₹349', emoji: '🌌', bg: '#ede7f6' },
  { id: 4, title: 'Retro Waves',     size: 'A3', price: '₹449', emoji: '🌊', bg: '#e3f2fd' },
  { id: 5, title: 'Midnight City',   size: 'A4', price: '₹329', emoji: '🏙', bg: '#e8eaf6' },
]
...
// ── localStorage helpers (persistent, never erases) ──────────────
function lsGet(key) {
...
// ── style tokens ─────────────────────────────────────────────────
const card  = { background:'#fff', border:'1.5px solid #efefef', borderRadius:12, padding:'14px' }
const label = { fontSize:11, fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'#c0c0c0', marginBottom:10, display:'block' }
const inp   = (err) => ({ width:'100%', padding:'9px 11px', border: err ? '1.5px solid #dc2743' : '1.5px solid #e0e0e0', borderRadius:8, fontSize:14, fontFamily:'inherit', boxSizing:'border-box', background:'#fff', outline:'none' })
const divider = { height:1, background:'#f0f0f0', margin:'18px 0' }
const igBtn = { padding:'10px 20px', border:'none', borderRadius:9, background:'linear-gradient(135deg,#f09433,#dc2743,#bc1888)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', display:'inline-flex', alignItems:'center', gap:7, fontFamily:'inherit' }
const ghost = { padding:'9px 16px', border:'1.5px solid #e5e5e5', borderRadius:8, background:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', color:'#444', fontFamily:'inherit', display:'inline-flex', alignItems:'center', gap:6 }
const badge = (s) => ({ fontSize:11, fontWeight:700, padding:'3px 9px', borderRadius:20, background: s==='done'?'#e8f5e9':s==='cancelled'?'#fce4ec':'#fff0f2', color: s==='done'?'#2e7d32':s==='cancelled'?'#c62828':'#dc2743' })
...
function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    const savedPosters = lsGet('bp:posters')
    if (savedPosters?.length) setPosters(savedPosters)
    const savedOrders  = lsGet('bp:orders')
    if (savedOrders)           setOrders(savedOrders)
  }, [])

  // poll orders every 6s
  useEffect(() => {
    const t = setInterval(() => {
      const o = lsGet('bp:orders')
      if (o) setOrders(o)
    }, 6000)
    return () => clearInterval(t)
  }, [])

  function savePosters(list) { setPosters(list); lsSet('bp:posters', list) }

  function markStatus(id, status) {
    const updated = orders.map(o => o.id === id ? { ...o, status } : o)
    setOrders(updated); lsSet('bp:orders', updated)
  }

  function addPoster() {
    if (!newP.title.trim() || !newP.price.trim()) return
    const p = { ...newP, id: Date.now(), size: newP.size.split(' ')[0] }
    savePosters([...posters, p])
    setSelected(p); setAdding(false)
  ...
  function shareWA() {
    if (!link) return
    const msg = encodeURIComponent(`Hi! Here's your order form for the poster 👇\n${link}`)
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", maxWidth:580, margin:'0 auto', padding:'24px 16px 80px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:24 }}>
        ...
        <span style={{ marginLeft:'auto', fontSize:11, background:'#fff0f2', color:'#c0234b', border:'1px solid #f5b3c0', borderRadius:20, padding:'3px 10px', fontWeight:700 }}>SELLER</span>
      </div>
      ...
      {/* ── SHARE TAB ── */}
      {tab === 'share' && <>
        <span style={label}>1 — Pick the poster they asked about</span>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(128px,1fr))', gap:10, marginBottom:20 }}>
          {posters.map(p => (
            <div key={p.id} onClick={() => setSelected(p)} style={{ ...card, border: selected?.id===p.id ? '2px solid #dc2743':'1.5px solid #efefef', background: selected?.id===p.id ? '#fff8f9':'#fff', cursor:'pointer' }}>
        ...
        {adding && (
          <div style={{ ...card, marginBottom:20 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:10 }}>
              <div><div style={{ fontSize:12, color:'#888', marginBottom:4 }}>Title *</div><input value={newP.title} onChange={e=>setNewP(p=>({...p,title:e.target.value}))} placeholder="Poster name" style={inp()} /></div>
              <div><div style={{ fontSize:12, color:'#888', marginBottom:4 }}>Price *</div><input value={newP.price} onChange={e=>setNewP(p=>({...p,price:e.target.value}))} placeholder="₹299" style={inp()} /></div>
            </div>
            ...
            <div style={{ marginBottom:10 }}>
              ...
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {EMOJI_OPTIONS.map(e=><div key={e} onClick={()=>setNewP(p=>({...p,emoji:e}))} style={{ width:32, height:32, borderRadius:7, border: newP.emoji===e?'2px solid #dc2743':'1px solid #e5e5e5', display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, cursor:'pointer' }}>{e}</div>)}
              </div>
            </div>
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:12, color:'#888', marginBottom:6 }}>Card colour</div>
              <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                {BG_OPTIONS.map(bg=><div key={bg} onClick={()=>setNewP(p=>({...p,bg}))} style={{ width:24, height:24, borderRadius:'50%', background:bg, border: newP.bg===bg?'2.5px solid #dc2743':'1.5px solid #ddd', cursor:'pointer' }} />)}
              </div>
            </div>
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={addPoster} style={{ flex:1, padding:'9px 0', background:'#dc2743', color:'#fff', border:'none', borderRadius:8, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Add poster</button>
        ...
        {selected && (
          <div style={{ marginTop:4 }}>
            <span style={label}>2 — Share this link</span>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:10 }}>
              <button onClick={copyLink} style={ghost}>{copied ? '✓ Copied!' : '📋 Copy link'}</button>
              <button onClick={shareIG} style={igBtn}>📩 Instagram DM</button>
              <button onClick={shareWA} style={{ ...ghost, borderColor:'#c8e6c9', color:'#2e7d32' }}>💬 WhatsApp</button>
            </div>
            <div style={{ fontSize:12, color:'#bbb', lineHeight:1.8 }}>
              This link <strong style={{ color:'#888' }}>never changes</strong> — even when you update the site. Customer fills the form → gets order ID → returns to @{IG_HANDLE}.
            </div>
          </div>
        )}
      </>}
      ...
      {tab === 'orders' && <>
        {orders.length === 0 ? (
          <div style={{ textAlign:'center', padding:'48px 0' }}>
            ...
            <div style={{ fontSize:13, color:'#aaa' }}>Once a customer submits the form, their order appears here permanently.</div>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {[...orders].reverse().map(o => (
              <div key={o.id} style={{ ...card, cursor:'pointer' }} onClick={()=>setExpanded(expanded===o.id?null:o.id)}>
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:3, flexWrap:'wrap' }}>
                      ...
                      <span style={badge(o.status)}>{o.status==='done'?'Done':o.status==='cancelled'?'Cancelled':'New'}</span>
                    </div>
                  ...
                  <div style={{ fontSize:16, color:'#ddd' }}>{expanded===o.id?'▲':'▼'}</div>
                </div>
                ...
                {expanded===o.id && (
                  <div style={{ marginTop:14, paddingTop:14, borderTop:'1px solid #f0f0f0' }}>
                    ...
                    {o.notes && <div style={{ fontSize:13, marginBottom:10 }}><span style={{ color:'#aaa' }}>Notes: </span><span style={{ color:'#111' }}>{o.notes}</span></div>}
                    <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
                      <button onClick={e=>{e.stopPropagation();markStatus(o.id,'done')}} style={{ ...ghost, color:'#2e7d32', borderColor:'#c8e6c9' }}>✓ Mark done</button>
                      <button onClick={e=>{e.stopPropagation();markStatus(o.id,'new')}} style={ghost}>↩ Mark new</button>
                      <button onClick={e=>{e.stopPropagation();markStatus(o.id,'cancelled')}} style={{ ...ghost, color:'#c62828', borderColor:'#ffcdd2' }}>✕ Cancel</button>
                    </div>
                  </div>
                )}
              </div>
...
function CustomerForm({ poster }) {
  ...
  function submit() {
    ...
    setBusy(true)
    const id = genOrderId()
    const order = { id, ...form, poster, status:'new', date: new Date().toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'}) }
    const existing = lsGet('bp:orders') || []
    lsSet('bp:orders', [...existing, order])
    setOrderId(id)
    setBusy(false)
  }
  ...
  if (orderId) return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", maxWidth:440, margin:'0 auto', padding:'60px 16px', textAlign:'center' }}>
      ...
      <button onClick={()=>window.open(IG_URL,'_blank')} style={igBtn}>Back to @{IG_HANDLE} on Instagram</button>
    </div>
  )

  const field = (k, placeholder, type='text') => (
    <input type={type} value={form[k]} onChange={e=>set(k,e.target.value)} placeholder={placeholder} style={{ ...inp(errors[k]), width:'100%' }} />
  )
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", maxWidth:460, margin:'0 auto', padding:'24px 16px 60px' }}>
      ...
      <span style={label}>Your details</span>
      ...
      <div style={divider} />
      <span style={label}>Size and notes</span>
      <div style={{ marginBottom:10 }}>
...
// ════════════════════════════════════════════════════════════════
// ROOT — reads URL to decide seller vs customer
// ════════════════════════════════════════════════════════════════
function AppRoot() {
